...
async function getSettings(){
    for(const k of Object.keys(settings)){
        ...
        const data = await cachedFetch('/php/dbReader.php?q=siteSettings&group=' + k, 'siteSettings');
        if(data.success){
...
export function initServiceOrderForm(serviceData) {
    ...
    title.textContent = "Konektem - service order";
    ...
    function setupFormHandlers(serviceData) {
        ...
        if (phoneInput) {
            ...
            phoneInput.addEventListener('blur', function() {
                // update this to the length of the local phone number
...
export default async function serviceForm(id){
    ...
    let serviceData = await cachedFetch('/php/dbReader.php?r=services', 'servicesData');
