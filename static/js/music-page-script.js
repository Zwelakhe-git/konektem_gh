...
function makeMusicItem(data) {
    ...
    const imageHtml = imageUrl 
        ? `<img class="full-w-h" alt="track image" src="${imageUrl}" onerror="this.style.display='none'; this.parentElement.querySelector('.fallback-icon').style.display='flex';" />`
        : '';
    ...
    div.innerHTML = `
        ...
        <div class="track-img">
            ${imageHtml}
            <div class="fallback-icon" style="display: ${imageUrl ? 'none' : 'flex'}; width: 100%; height: 100%; align-items: center; justify-content: center; background: linear-gradient(135deg, #3498db, #2c3e50); color: white; font-size: 2rem; border-radius: 10px;">
                <i class="fas fa-music"></i>
            </div>
        </div>
        <div class="music-info">
            <div class="music-title">${data.title || 'Untitled'}</div>
            <div class="music-artist">${data.artist_name || 'Unknown Artist'}</div>
