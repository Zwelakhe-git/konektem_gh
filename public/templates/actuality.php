...
<div class='container' id='current-news-root'></div>
    ...
    <?php
        ...
        foreach($newsCategories as $cat){
            $targetArticles = array_filter($news, fn($article) => $article['newsCategory'] === $cat['name']);
    ?>
    <section id="<?= $cat['id']?>" class="news-section">
        ...
        <div class="news-grid">
            ...
            <div class="news-card" id="<?= $article['id'] ?>">
                <div class="news-img">
                    <img src="<?= $article['image_location'] ?>" alt="<?= $cat['name']?> news">
                </div>
                <div class="news-content">
                    <div class="news-date"><?= $article['newsDate'] ?></div>
                    <a><h3 class="news-title"><?= $article['newsTitle'] ?></h3></a>
                    <p class="news-excerpt"><?= $article['newsHeadline'] ?></p>
